...
export type NoteNodeProps = NodeProps<NoteNodeType>;

export function NoteNode({ id, selected, data, deletable }: NoteNodeProps) {
	const updateNode = useWorkflow((store) => store.updateNode);
	const deleteNode = useWorkflow((store) => store.deleteNode);
	...
	return (
		<ResizableNode
			...
			nodeId={id}
			deletable={deletable}
			onDelete={deletable ? () => deleteNode(id) : undefined}
			className="p-4"
		>
			<Textarea
				value={data.content}
